  ...
  <div class="detail">
    ...
    <section class="con">
      ...
      <div class="conPoin">
        ...
        <span v-if="categoryView.category4Name">{{
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a href="javascript:" class="mins" @click="buyNum<=1 ? buyNum=1 : buyNum--">-</a>
              </div>
              <div class="add">
...
import Zoom from "./Zoom/ZoomIndex";
// 从 vuex 中拉数据
...
export default {
  ...
  computed: {
    ...
    // 给子组件的数据，这个数据有可能为 undefined
    skuImageList() {
      return this.skuInfo.skuImageList || [];
      // skuImageList 是从父组件传来的，当父组件的数据还没有来时，为 undefined, 所以我们要考虑这种情况
    },
  },
  methods:{
    changeActive(spuSaleAttrValue, spuSaleAttrValueList){
      ...
      // 注意，不仅要把点击的这个元素的 isChecked 赋 1，这个元素的兄弟元素还要 赋 0
      // 重点：先所有的都赋值0 再 把选中的赋值 1
    },
    // 修改购买产品个数的方法
    changeBuyNum(event){
    ...
    // 跳转到成功加入购物车的组件方法
    async addShopcar(){
      // 发送请求，将产品添加到数据库,并跳转路由
      // this.$router.push('/addcartsuccess', query:{})
      try {
        await this.$store.dispatch("addOrUpdateShopCart", {skuId:this.$route.params.skuid, skuNum:this.buyNum})
        // 进行路由跳转
        // 这里路由跳转，需要带的参数，比较复杂，是一个商品对象的全部信息，还有这个商品的数量，商品对象我们一般用会话存储
        sessionStorage.setItem('SKUINFO', JSON.stringify(this.skuInfo));
        this.$router.push({name:'addcartsuccess', query:{skuNum: this.buyNum}})
      } catch (error) {
        alert(error.message);
      }
    }
