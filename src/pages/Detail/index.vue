          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd changepirce="0" :class="{active: spuSaleAttrValue.isChecked == '1'}" v-for="spuSaleAttrValue in spuSaleAttr.spuSaleAttrValueList" :key="spuSaleAttrValue.id" @click="changeSaleAttr(spuSaleAttrValue, spuSaleAttr.spuSaleAttrValueList)">{{ spuSaleAttrValue.saleAttrValueName }}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt"  v-model="skuNum" ref="inputNum" @change="inputNum"/>
                <a href="javascript:" class="plus" @click="skuNum++">+</a>
                <a href="javascript:" class="mins" @click="skuNum<=1 ? skuNum=1 : skuNum--">-</a>
              </div>
              <div class="add">
                <a href="javascript:" @click="addUpdateGoods">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    // 发送请求获取详情页数据,注意携带参数（在路由信息中有）
    this.$store.dispatch("getDetail", this.$route.params.skuId);
  },
  methods: {
    changeSaleAttr(arrElement, arr) {
      arr.forEach(item => {
        item.isChecked = 0;
      });
      arrElement.isChecked = 1;
    },
    inputNum() {
      // 这个方法是 在 数量输入框的值改变时触发，change 触发的时候已经失去焦点
      let inputNum =  this.$refs.inputNum;
      if (isNaN(inputNum.value * 1)) {
        this.skuNum = 1;
      }else {
        this.skuNum = parseInt(inputNum.value) < 1 ? 1 : parseInt(inputNum.value);
      }
    },
    async addUpdateGoods() {
      // 这个方法是在点击 加入购物车时触发。
      // 路由跳转，并把添加的信息post 给服务器。
      try {
        await this.$store.dispatch('addUpdateGoods', {skuId: this.$route.params.skuId, skuNum: this.skuNum});
        // 把我们要传给 addCartSuccess 组件的数据放在会话存储中。
