...
const { detailInfo, categoryView, skuInfo} = storeToRefs(detailStore);
const skuImageList = computed(() => skuInfo.value.skuImageList || []);

async function addShopCar() {
  try {
    await shopCartStore.addUpdateGoods(route.params.skuId, skuNum.value);
    // 把这个商品的信息存储在 sessionStorage中，方便在加入购物车成功页面获取这个商品的信息，比如图片和描述进行展示
    sessionStorage.setItem('skuInfo', JSON.stringify(skuInfo.value))

    // 如果加入购物车成功，就进行路由跳转，跳转到加入购物车成功的页面。
    router.push(`/addcartsuccess/${skuNum.value}`)
  } catch (error) {
    alert(error.message);
  }
}

function changeSaleAttr(attrValue, attrValueList) {
...
function changeNum() {
  if (isNaN(inputNum.value.value * 1)) {
    skuNum.value = 1;
  }else {
    skuNum.value = parseInt(inputNum.value.value) < 1 ? 1 : parseInt(inputNum.value.value)
  }
}

onMounted(() => {
  detailStore.getDeatilInfo(route.params.skuId);
})
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in detailInfo.spuSaleAttrList">
                ...
                <dd changepirce="0" :class="{active:spuSaleAttrValue.isChecked == '1'}"
                  ...
                  @click="changeSaleAttr(spuSaleAttrValue, spuSaleAttr.spuSaleAttrValueList)"
                >{{spuSaleAttrValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" ref="inputNum" @change="changeNum">
                <a @click="skuNum++" class="plus">+</a>
                <a @click="skuNum<=1 ? skuNum=1 : skuNum--" class="mins">-</a>
              </div>
              <div class="add">
