...
const { myOrder, myOrderList } = storeToRefs(centerStore);

const pageNo = ref(1);
const limit = ref(3);

async function changePage(page) {
  // 拿到当前页后再次发送请求，请求这个页的数据
  try {
    await centerStore.getMyOrderList(page, limit.value);
    pageNo.value = page;
  } catch (error) {
    console.error(error);
  }
...
onMounted(async () => {
  try {
    await centerStore.getMyOrderList(pageNo.value, limit.value);
  } catch (error) {
    console.error(error);
  }
})
  ...
  <div class="order-right">
    <div class="order-content">
      ...
      <div class="choose-order">
        <Pagination :pageNo="pageNo" :total="myOrder.total" :pageSize="limit" :pageContinue="5" @change-page="changePage">
        </Pagination>
      </div>
    </div>
    <!--猜你喜欢-->
